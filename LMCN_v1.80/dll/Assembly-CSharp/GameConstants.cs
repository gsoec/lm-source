...
public static class GameConstants
{
	...
	// Token: 0x06000717 RID: 1815 RVA: 0x0009F60C File Offset: 0x0009D80C
	public unsafe static GameObject GameObjectPick(Vector2 touch, GameObject[] in_gameObjects, Type t, bool Overlay = false)
	{
		GameConstants.inv_direction.Set(touch.x, touch.y, 0f);
		Ray ray = Camera.main.ScreenPointToRay(GameConstants.inv_direction);
		int* ptr;
		Vector3* ptr2;
		checked
		{
			ptr = stackalloc int[3 * 4];
			ptr2 = stackalloc Vector3[2 * sizeof(Vector3)];
			if (Input.touchCount > 1)
			{
				return null;
			}
			GameConstants.inv_direction.Set(1f / ray.direction.x, 1f / ray.direction.y, 1f / ray.direction.z);
			*ptr = ((GameConstants.inv_direction.x >= 0f) ? 0 : 1);
		}
		ptr[1] = ((GameConstants.inv_direction.y >= 0f) ? 0 : 1);
		ptr[2] = ((GameConstants.inv_direction.z >= 0f) ? 0 : 1);
		float num = 65536f;
		GameObject result = null;
		byte b = (byte)(in_gameObjects.Length - 1);
		for (short num2 = (short)b; num2 >= 0; num2 -= 1)
		{
			if (in_gameObjects[(int)num2] && in_gameObjects[(int)num2].activeSelf)
			{
				Renderer renderer = in_gameObjects[(int)num2].GetComponent(t) as Renderer;
				if (renderer == null && in_gameObjects[(int)num2].transform.childCount > 0)
				{
					renderer = (in_gameObjects[(int)num2].transform.GetChild(0).GetComponent(t) as Renderer);
				}
				if (renderer == null)
				{
					renderer = (in_gameObjects[(int)num2].GetComponentInChildren(t) as Renderer);
				}
				if (!(renderer == null) && renderer.enabled)
				{
					*ptr2 = renderer.bounds.min;
					ptr2[1] = renderer.bounds.max;
					float num3 = (ptr2[*ptr].x - ray.origin.x) * GameConstants.inv_direction.x;
					float num4 = (ptr2[1 - *ptr].x - ray.origin.x) * GameConstants.inv_direction.x;
					float num5 = (ptr2[ptr[1]].y - ray.origin.y) * GameConstants.inv_direction.y;
					float num6 = (ptr2[1 - ptr[1]].y - ray.origin.y) * GameConstants.inv_direction.y;
					if (num3 <= num6 && num5 <= num4)
					{
						if (num5 > num3)
						{
							num3 = num5;
						...
						float num7 = (ptr2[ptr[2]].z - ray.origin.z) * GameConstants.inv_direction.z;
						float num8 = (ptr2[1 - ptr[2]].z - ray.origin.z) * GameConstants.inv_direction.z;
						if (num3 <= num8 && num7 <= num4)
						{
							if (num7 > num3)
							{
								num3 = num7;
							...
							{
								num = num3;
								result = in_gameObjects[(int)num2];
								if (!Overlay)
								{
									break;
								}
	...
	// Token: 0x0600071B RID: 1819 RVA: 0x0009FB88 File Offset: 0x0009DD88
	public static void FormatResourceValue(CString CStr, uint value)
	{
		if (value >= 1000000000u)
		{
			CStr.FloatToFormat((float)(value / 1000000000.0), 2, false);
			CStr.AppendFormat("{0}B");
		}
		else if (value >= 100000000u)
		{
			CStr.IntToFormat((long)((ulong)(value / 1000000u)), 1, false);
			CStr.AppendFormat("{0}M");
		}
		else if (value >= 10000000u)
		{
			CStr.FloatToFormat(value / 1000000f, 1, false);
			CStr.AppendFormat("{0}M");
		}
		else if (value >= 1000000u)
		{
			CStr.FloatToFormat(value / 1000000f, 2, false);
			CStr.AppendFormat("{0}M");
		}
		else if (value >= 100000u)
		{
			CStr.IntToFormat((long)((ulong)(value / 1000u)), 1, false);
			CStr.AppendFormat("{0}K");
		}
		else if (value >= 10000u)
		{
			CStr.FloatToFormat(value / 1000f, 1, false);
			CStr.AppendFormat("{0}K");
		}
		else if (value >= 1000u)
		{
			CStr.IntToFormat((long)((ulong)(value / 1000u)), 1, false);
			CStr.IntToFormat((long)((ulong)(value % 1000u)), 3, false);
			CStr.AppendFormat("{0},{1}");
		}
		else
		{
			CStr.IntToFormat((long)((ulong)value), 1, false);
			CStr.AppendFormat("{0}");
		}
	}

	// Token: 0x0600071C RID: 1820 RVA: 0x0009FCFC File Offset: 0x0009DEFC
	...
	// Token: 0x06000721 RID: 1825 RVA: 0x000A0050 File Offset: 0x0009E250
	public static bool IsPetSkillLine(int lineTableID)
	{
		return (DataManager.MapDataController.MapLineTable[lineTableID].baseFlag & 2) != 0;
	}

	// Token: 0x06000722 RID: 1826 RVA: 0x000A0070 File Offset: 0x0009E270
	public static bool IsSoccerRunningLine(int lineTableID)
	{
		return (DataManager.MapDataController.MapLineTable[lineTableID].baseFlag & 4) != 0;
	}
	...
	// Token: 0x0600072F RID: 1839 RVA: 0x000A04E8 File Offset: 0x0009E6E8
	public static void GetEffectValue(StringBuilder sb, ushort meffectId, uint mValue = 0u, byte Kind = 0, float fValue = 0f, long mValue2 = 0L)
	{
		sb.Length = 0;
		...
		case 1:
		{
			...
			GameConstants.FormatValue(stringBuilder, mValue);
			sb.AppendFormat(DataManager.Instance.mStringTable.GetStringByID((uint)recordByKey.String_infoID), stringBuilder);
			break;
		}
		case 2:
			...
			sb.AppendFormat("{0:N0}", mValue);
			sb.Append(DataManager.Instance.mStringTable.GetStringByID((uint)recordByKey.ValueID));
			break;
		case 3:
			if (meffectId > 1000)
			{
				sb.AppendFormat("{0}", DataManager.Instance.mStringTable.GetStringByID((uint)recordByKey.InfoID));
			}
			else
			{
				if (meffectId == 0)
		...
		case 7:
			if (meffectId > 1000)
			{
				sb.AppendFormat("{0}", DataManager.Instance.mStringTable.GetStringByID((uint)recordByKey.String_infoID));
			}
			else
			{
				if (meffectId == 0)
				...
				if (recordByKey.ValueID == 4378)
				{
					if (mValue2 != 0L)
					{
						...
						sb.AppendFormat("{0:#0.##%}", fValue / 100f);
					}
					else
					{
						fValue = mValue / 100f;
	...
	// Token: 0x06000730 RID: 1840 RVA: 0x000A0864 File Offset: 0x0009EA64
	public static void GetEffectValue(CString CStr, ushort meffectId, uint mValue = 0u, byte Kind = 0, float fValue = 0f)
	{
		CStr.ClearString();
		Effect recordByKey = DataManager.Instance.EffectData.GetRecordByKey(meffectId);
		switch (Kind)
		...
		case 0:
			...
			break;
		case 1:
			CStr.IntToFormat((long)((ulong)mValue), 1, true);
		...
		case 5:
			...
			CStr.FloatToFormat(fValue, -1, true);
			if (recordByKey.ValueID != 4378)
			{
				if (GUIManager.Instance.IsArabic)
				{
		...
		case 6:
			...
			CStr.FloatToFormat(fValue, -1, false);
			CStr.AppendFormat("{0}");
			CStr.Append(DataManager.Instance.mStringTable.GetStringByID((uint)recordByKey.ValueID));
			break;
		case 7:
		...
		case 11:
			...
			CStr.FloatToFormat(mValue, -1, true);
			if (recordByKey.StatusIcon == 0 || recordByKey.StatusIcon == 2)
			{
				CStr.AppendFormat("+{0}");
			}
			else
	...
	// Token: 0x06000731 RID: 1841 RVA: 0x000A0EE0 File Offset: 0x0009F0E0
	...
	// Token: 0x06000732 RID: 1842 RVA: 0x000A0FC4 File Offset: 0x0009F1C4
	public static void GetTimeString(CString CStr, uint sec, bool useSpace = false, bool hideTimeIfDays = false, bool showZeroHour = true, bool withArabic = false, bool bShowDay = true)
	{
		uint num = sec;
		if (!GUIManager.Instance.IsArabic || !withArabic)
		{
			...
			{
				CStr.IntToFormat((long)(num / 3600u), 2, false);
				if (useSpace)
				{
					CStr.AppendFormat("{0} : ");
				}
		...
		else
		{
			...
			if (bShowDay && num > 86400u)
			{
				...
				if (hideTimeIfDays)
				{
					CStr.IntToFormat((long)num2, 1, false);
					CStr.AppendFormat("{0}d");
					return;
				}
				num %= 86400u;
	...
	// Token: 0x06000733 RID: 1843 RVA: 0x000A11B8 File Offset: 0x0009F3B8
	...
	// Token: 0x06000734 RID: 1844 RVA: 0x000A12A8 File Offset: 0x0009F4A8
	public static void GetTimeStringShort(CString CStr, uint sec)
	{
		...
		num %= 60u;
		CStr.IntToFormat((long)((ulong)num), 1, false);
		CStr.AppendFormat("{0}s");
	}

	// Token: 0x06000735 RID: 1845 RVA: 0x000A1354 File Offset: 0x0009F554
	public static void GetNameString(CString CStr, ushort KingdomID, CString Name, CString AlliTag, bool alwaysShowKingdom = false)
	{
		GameConstants.FormatRoleName(CStr, Name, AlliTag, null, 0, (!alwaysShowKingdom && (KingdomID == 0 || KingdomID == DataManager.MapDataController.kingdomData.kingdomID)) ? 0 : KingdomID, null, null, null, null);
	}

	// Token: 0x06000736 RID: 1846 RVA: 0x000A139C File Offset: 0x0009F59C
	public static void GetAllianceNameString(CString CStr, ushort KingdomID, CString Name, CString AlliTag, bool alwaysShowKingdom = false)
	{
		if (DataManager.Instance.RoleAlliance.Id > 0u)
		{
			GameConstants.FormatRoleName(CStr, Name, AlliTag, null, 0, (!alwaysShowKingdom && (KingdomID == 0 || KingdomID == DataManager.Instance.RoleAlliance.KingdomID)) ? 0 : KingdomID, null, null, null, null);
		}
		else
		{
			GameConstants.FormatRoleName(CStr, Name, AlliTag, null, 0, KingdomID, null, null, null, null);
		}
	}
	...
	// Token: 0x06000738 RID: 1848 RVA: 0x000A147C File Offset: 0x0009F67C
	public static void GetColoredLordEquipString(CString CStr, ushort ItemID, byte color)
	{
		...
		GameConstants.GetColoredLordEquipString(CStr, ref recordByKey, color);
	}

	// Token: 0x06000739 RID: 1849 RVA: 0x000A14A4 File Offset: 0x0009F6A4
	public static void GetColoredLordEquipString(CString CStr, ref Equip itemData, byte color)
	{
		...
		CStr.Append(DataManager.Instance.mStringTable.GetStringByID((uint)itemData.EquipName));
		CStr.Append("</color>");
	}

	// Token: 0x0600073A RID: 1850 RVA: 0x000A14F4 File Offset: 0x0009F6F4
	public static void GetKingdomXYString(CString CStr, ushort KingdomID, int MapID, bool NoKingdomID = false)
	{
		if (!GUIManager.Instance.IsArabic)
		{
			if (!NoKingdomID && KingdomID != 0)
			{
				CStr.IntToFormat((long)KingdomID, 1, false);
				CStr.AppendFormat("K:{0} ");
			}
			Vector2 tileMapPosbyMapID = GameConstants.getTileMapPosbyMapID(MapID);
			CStr.IntToFormat((long)tileMapPosbyMapID.x, 1, false);
			CStr.IntToFormat((long)tileMapPosbyMapID.y, 1, false);
			CStr.AppendFormat("X:{0} Y:{1}");
		}
		else
		{
			Vector2 tileMapPosbyMapID2 = GameConstants.getTileMapPosbyMapID(MapID);
			CStr.IntToFormat((long)tileMapPosbyMapID2.x, 1, false);
			CStr.IntToFormat((long)tileMapPosbyMapID2.y, 1, false);
	...
	// Token: 0x0600073E RID: 1854 RVA: 0x000A1670 File Offset: 0x0009F870
	...
	// Token: 0x0600073F RID: 1855 RVA: 0x000A16C0 File Offset: 0x0009F8C0
	...
	// Token: 0x06000741 RID: 1857 RVA: 0x000A1768 File Offset: 0x0009F968
	public static bool FormatRoleName(CString FromattedName, CString Name, CString Tag = null, CString Nickname = null, byte bCheckedNickname = 0, ushort KingdomID = 0, string KingdomColor = null, string NameColor = null, string TagColor = null, string NickColor = null)
	{
		bool flag = false;
	...
	// Token: 0x06000742 RID: 1858 RVA: 0x000A1AE4 File Offset: 0x0009FCE4
	...
	// Token: 0x06000743 RID: 1859 RVA: 0x000A1BA8 File Offset: 0x0009FDA8
	public static ushort RandomValue(ushort seed, byte gap, ushort count)
	{
		return GameConstants.RandomTB[(int)((seed + (ushort)gap * count) % 1000)];
	}

	// Token: 0x06000744 RID: 1860 RVA: 0x000A1BBC File Offset: 0x0009FDBC
	public static bool IsMarchDeparture(EMarchEventType Type)
	{
		return (Type >= EMarchEventType.EMET_AttackMarching && Type <= EMarchEventType.EMET_DeliverMarching) || Type == EMarchEventType.EMET_FooballMarching;
	}

	// Token: 0x06000745 RID: 1861 RVA: 0x000A1BD8 File Offset: 0x0009FDD8
	public static bool IsMarchReturnOrRetreat(EMarchEventType Type)
	{
		return (Type >= EMarchEventType.EMET_AttackReturn && Type <= EMarchEventType.EMET_HitMonsterRetreat) || Type == EMarchEventType.EMET_FooballReturn || Type == EMarchEventType.EMET_FooballRetreat;
	}
